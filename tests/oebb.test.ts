...
import { uicEnvelope, uicRecord } from './helpers/build.ts';
...
describe('OeBB validity record', () => {
	const payload = uicEnvelope(1181, uicRecord('118199', 1, VALIDITY));
	...
	it('keeps unknown keys instead of dropping them', () => {
		const withExtra = parsePayload(
			uicEnvelope(1181, uicRecord('118199', 1, '{"V":"2405110730","B":"2405120730","X":"7"}'))
		);
	...
	it('parses a bare "{}" as an empty record rather than failing', () => {
		const bare = parsePayload(uicEnvelope(1181, uicRecord('118199', 1, '{}')));
