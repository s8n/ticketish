<script lang="ts">
	...
	import { identityProblem } from '../wallet/identity.ts';
	...
	import WalletCredentialsForm from './WalletCredentialsForm.svelte';
	...
	{#if setup}
		<WalletCredentialsForm
			...
			onsaved={() => {
				const saved = setup;
				setup = null;
				return saved === 'apple' ? exportApple() : exportGoogle();
			}}
		/>
	{/if}
{/if}
